...
Write-Host "[*] Patching ProjectLock.cs (Thread-safe FileStream lock)..." -ForegroundColor Cyan
...
Write-Host "[*] Patching WindowsFirewallEnforcer.cs (Elevation detection)..." -ForegroundColor Cyan
Set-Content -Path (Join-Path $Root "src/Sevelr/Network/WindowsFirewallEnforcer.cs") -Value @'
...
Write-Host "[*] Rebuilding Sevelr single-file binary..." -ForegroundColor Cyan
...
Write-Host "`n[✓] Successfully patched, re-compiled, and packaged!" -ForegroundColor Green